#!/usr/bin/env python3
"""Run and resume Ergouzi video prediction tasks."""
...
from media import LOCAL_FILE_KEY, MODEL_MEDIA_FIELDS, resolve_media_inputs
...
def _read_input(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.prompt is not None and args.model not in PROMPT_MODELS:
        raise ClientError(
            f"--prompt is not valid for {args.model}; use --input-file or --input-json"
        )
    if args.prompt is not None:
        value["prompt"] = args.prompt
    if args.image:
        if args.model == "ergouzi/e-video-replace":
            value["images"] = [_media_argument(item) for item in args.image]
        elif len(args.image) == 1:
            value["image"] = _media_argument(args.image[0])
        else:
            raise ClientError(f"{args.model} accepts one --image")
...
def _output_path(value: str | None) -> Path | None:
    ...
    if path.suffix.lower() != ".mp4":
        raise ClientError("Video output path must end in .mp4")
    if path.exists():
...
def _complete(
    ...
    files = (
        download_outputs(
            ...
            expected_media_types=frozenset({"video/mp4"}),
        )
...
def parse_args() -> argparse.Namespace:
    ...
    predict.add_argument("--video", help="Local MP4 path or HTTPS URL")
    predict.add_argument("--audio", help="Local audio path or HTTPS URL")
    predict.add_argument("--last-frame-image", help="Local image path or HTTPS URL")
    predict.add_argument("--output-dir", default=DEFAULT_OUTPUT_DIR)
    predict.add_argument("--output", help="Exact .mp4 output path")
    predict.add_argument("--timeout-seconds", type=int, default=3600)
    ...
    status.add_argument("--output", help="Exact .mp4 output path")
    status.add_argument("--timeout-seconds", type=int, default=3600)
